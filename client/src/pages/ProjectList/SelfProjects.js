...
async function cancelProject(projectId) {
  const deleteProject = await axios(
    `http://localhost:5000/project/cancel/${projectId}`,
    {
      ...config,
      method: "PUT",
    }
              ...
              key={key}>
                ...
                >
                  ...
                  {currentUser &&
                      ...
                      (project.status != "Created" && currentUser.role === "user") &&
                      project.status != "ProcessDesigner" &&
                      !(project.state === "InProgress" && currentUser.role === "user") &&
                      !(project.status === "ApprovedPChecker") && (
                        <div className="col-sm text-right pt-5" style={{ marginTop: "-50px"}}>
                          <button
                            onClick={() => {
                    ...
                    {currentUser &&
                      currentUser.role == "user" &&
                      (project.status == "SubmittedDesigner" && currentUser.role === "user") &&
                      !(
                    ...
                    {currentUser &&
                      roleHasCancelPermission.includes(currentUser.role) && !(project.state === "Completed") &&(
                        <div className="col-sm text-right pt-5" style={{ marginTop: "-50px"}}>
                          <button
                            onClick={() => {
